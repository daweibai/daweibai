...
jsPsych.plugins.animation = (function() {
  ...
  plugin.info = {
	...
	responded_prompt: { // added by AF
            type: jsPsych.plugins.parameterType.STRING,
            pretty_name: 'Prompt if responded',
            default: null,
            description: 'This will replace the existing prompt if a keypress response has been received.'
	},
	flip_x: { // added by AF
	    type: jsPsych.plugins.parameterType.BOOL,
	    pretty_name: 'Flip horizontally.',
	    default: false,
	    description: 'If true, all stimuli will be shown flipped along the X axis.'
	},
	display: { // added by AF
	    type: jsPsych.plugins.parameterType.BOOL,
	    pretty_name: 'Display animation or not.',
	    default: true,
	    description: 'If false, animation will not display (useful for preloading).'
	},
    }
  }
  ...
  plugin.trial = function(display_element, trial) {
    ...
    function show_next_frame() {
      // show image
	// flip logic added by AF
	display_element.innerHTML = ( (trial.flip_x)? '<style>img {-webkit-transform: scaleX(-1); transform: scaleX(-1); }</style>' : '' ) +
	    ((trial.display)? '' : '<style>img { visibility: hidden; }</style>') +
	...
	if (trial.responded_prompt !== null & responses.length > 0){
	    display_element.innerHTML += trial.responded_prompt; // added by AF
	} else if (trial.prompt !== null) {
