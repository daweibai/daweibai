// Re-pilot of exp 1
// With perfect randomized position
// by dawei 03/2021
...
const task_version    = 'Exp 1 - good random';
...
let ballTooCloseToCenter, ballTooFast, targetTooFast
let isTraining
...
var trainArrayShuffled 	   = jsPsych.randomization.repeat(trainArray, 2);
var conditionArrayShuffled = jsPsych.randomization.repeat(conditionArray, 13);
...
function randomAngle() {
	return angleArray[Math.floor(Math.random() * 4)]
}
...
display = function() {    
	...
	function Ball(x, y, dx, dy, gravityDirection, target) {
		this.x0 = x; // starting position
		this.y0 = y;
		this.gravityDirection 	= gravityDirection
		this.target = target;
		this.isUnderGravity = true

		this.leftRight = Math.abs(dx)/dx // switches sign after reaching edge. +1 = going right. -1 = going left. 

		this.dx0 = dx // speed of the ball in NORMAL parabola (i.e. dy under g, dx constant)
		this.dy0 = dy

        // given x, y, dx, dy, g, what is the equation of the NORMAL parabola?
		// parameters of the parabola y = (x - a)^2/b + maxH
		this.maxH = this.y0 - this.dy0 * this.dy0 / (2* g) // c = vertex's y

		this.a = this.x0 + (this.dx0 * this.dy0)/g
		this.b = (this.x0 - this.a) * (this.x0 - this.a) / (this.y0 - this.maxH)
		
		this.vyMax = Math.sqrt(2* (canvas_height - r_ball - this.y0) * g + this.dy0 * this.dy0)
		this.t = this.vyMax/ g  // time (n of frames) per half of path

		// randomly draw a position on the trajectory
		this.y = randomIntFromRange(this.maxH + r_ball, canvas_height - r_ball);
		this.PlusMinus = Math.floor(Math.random() *2)*2 - 1 
		this.x = this.a + Math.sqrt((this.y - this.maxH) * this.b) * this.PlusMinus

		while ((this.x < 0 + r_ball) || (this.x > canvas_width - r_ball)) {
			this.PlusMinus = -1 * (this.PlusMinus)
			this.y = randomIntFromRange(this.maxH + r_ball, canvas_height - r_ball);
			this.x =  this.a + Math.sqrt((this.y - this.maxH) * this.b)  * this.PlusMinus
		}

		if (this.isUnderGravity) { 	// realistic trials
			this.dy = Math.sqrt(2* (this.y - this.maxH) * g) * ( Math.floor(Math.random() *2)*2 - 1 )
			this.dx = this.dx0
		} else {					// unrealistic trials
			// this.dy = (canvas_height - this.maxH)/this.t * Math.abs(this.dy0)/this.dy0
			this.dy = Math.sqrt(2* (canvas_height - r_ball - this.y) * g) * ( Math.floor(Math.random() *2)*2 - 1 )
		...
		this.update = function() {
			...
			if (this.isUnderGravity) {  // realistic trials
				...
				this.dy += -g
				this.dx = Math.sqrt(this.b) * Math.pow((this.y - this.maxH), -0.5) * Math.abs(this.dy) * this.leftRight *0.5
				// this.dx = this.dx0 * this.leftRight
				if ((Math.abs(this.dx) > 30) || (isNaN(this.dx))) {  // vertex
					this.dx = 30 * this.leftRight * Math.abs(this.dx0/10)
					this.dy = this.vyMax
				}
			}

			// lower and upper edges
			if ((this.y + r_ball + this.dy> canvas.height)||(this.y + this.dy<= 0 + r_ball)) { 
				if (!(this.isUnderGravity) && (this.y + r_ball + this.dy> canvas.height)) {
					this.dy = 0
				}
				
				this.dy = -this.dy
			// right and left edges
			} else if ((this.x + r_ball + this.dx >= canvas.width) || (this.x + this.dx <= 0 + r_ball)) { 
				this.dx = -this.dx;
				this.leftRight = -1 * this.leftRight
			} 


			// if ((this.y2 + r_ball + this.vy2> canvas.height)||(this.y2 + this.vy2<= 0 + r_ball)) { 
			// 	this.vy2 = -this.vy2
			// } else {

			// 	this.vy2 += g
			// }
			// if ((this.x2 + r_ball + this.dx0 >= canvas.width) || (this.x2 + this.dx0 <= 0 + r_ball)) { 
			// 	this.dx0 = -this.dx0;
			// } 
			...
			// all the above calculations are for the downward direction. Now transform them into 4 cardinal directions for drawing
            switch (this.gravityDirection) {
                ...
                case "up": 
                    this.drawX = this.x;
                    this.drawY = canvas.height - this.y;
                    break;
                case "left": 
                    this.drawX = canvas.width - this.y;
                    this.drawY = this.x;
                    break;
                case "right": 
                    this.drawX = this.y;
		...
		this.draw = function() {
			c.beginPath();
			c.arc(this.drawX, this.drawY, r_ball, 0, pi * 2, false);	
			...
			// c.beginPath();
			// c.arc(this.x2, this.y2, r_ball, 0, pi * 2, false);	
			// c.fillStyle = "green";
			// c.fill();
			// c.stroke();
			// c.closePath();
			...
			if (this.target == false) {
				c.beginPath();
				c.rect(this.drawX - cross_width/2, this.drawY - cross_length/2, cross_width, cross_length );
				c.rect(this.drawX - cross_length/2, this.drawY - cross_width/2, cross_length, cross_width );
				c.fillStyle = 'white';
				c.fill();
			} else {
			// // if target, then draw T
				c.beginPath();
				c.rect(this.drawX - cross_width/2, this.drawY - cross_length/2 + 2, cross_width, cross_length -2);
				c.rect(this.drawX - cross_length/2, this.drawY - cross_width/2 - 5, cross_length, cross_width );
				c.fillStyle = 'white';
				c.fill();


                targetPressedX = this.drawX
                targetPressedY = this.drawY
			}
		};
	}
	...
	function init() {
        ...
        for (let i = 0; i < n_balls; i++) {
            ...
            ballArray.push(new Ball(x, y, dx, dy, angle, false));
        }

        ballArray.push(new Ball(track_x, track_y, track_dx, track_dy, track_angle, true));
		jsPsych.data.write({track_angle: track_angle, recording: 'invisAngle', track_angle_invis: track_angle_invis, correct_key: correctKey, trial: trialCounter})
	}
